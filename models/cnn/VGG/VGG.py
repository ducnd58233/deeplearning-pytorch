...
class VGG(nn.Module):
    r"""
    Desc:
        Customize for MNIST and CIFAR10
...
def _vgg(cfg: str, batch_norm: bool, num_classes: int = 10, in_channels: int = 3, init_weights: bool = True, **kwargs: Any) -> VGG:
    r"""
    Args:
        cfg (str): config types defines in cfgs
        batch_norm (bool): If true, model will use batch normalization
        num_classes (int): Number of classes (output). Default: 10
        in_channels (int): Number of channels of the input. Default: 3 
    """
    
    return VGG(make_layers(cfgs[cfg], batch_norm=batch_norm, in_channels=in_channels),  **kwargs)
    
def vgg16(num_classes: int = 10, in_channels: int = 3, init_weights: bool = True, **kwargs):
    r"""
    Desc: 
        VGG16 without batch normalization. Already customs for MNIST and CIFAR
...
def vgg16_bn(num_classes: int = 10, in_channels: int = 3, init_weights: bool = True, **kwargs):
    r"""
    Desc: 
        VGG16 with batch normalization. Already customs for MNIST and CIFAR
